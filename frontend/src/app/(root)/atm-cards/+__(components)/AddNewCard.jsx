...
export default function AddNewCardDialog() {
  ...
  // Normalize account_no to always be an array
  const accounts = Array.isArray(user?.account_no)
    ? user.account_no
    : user?.account_no
      ? [{ _id: user.account_no._id || user.account_no, amount: user.amount ?? 0 }]
      : [];
  console.log('ATM Modal user.account_no:', user?.account_no);
  ...
  return (
    <> 
        <button
          type="button"
          onClick={openModal}
          className="rounded-md w-1/2 px-6 py-2 bg-blue-700 text-white flex items-center justify-center gap-x-2 outline-none border-none cursor-pointer"
        >
                                      ...
                                      <Field name="account" id="account" as="select" className="w-full py-2 border border-rose-500 rounded outline-none px-2">
                                        {accounts.length > 0 ? (
                                          <>
                                            ...
                                            {accounts.map((cur, i) => (
                                              <option key={i} value={cur._id}>{`${cur._id} - ₹${cur.amount}`}</option>
                                            ))}
                                          </>
                                        ) : (
