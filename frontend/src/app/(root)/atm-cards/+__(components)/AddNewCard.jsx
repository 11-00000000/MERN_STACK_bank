...
export default function AddNewCardDialog() {
  ...
  const [cards, setCards] = useState(user.atmCards || []); // local cards state

  const Types = Object.keys(CARD_TYPE);

  useEffect(() => {
    // update local cards if user context changes
    if (user.atmCards) setCards(user.atmCards);
  }, [user.atmCards]);
  ...
  const validationSchema = yup.object({
    ...
    card_type: yup.string()
      .required("Choose Valid Card Type")
      .oneOf(Object.keys(CARD_TYPE), "Choose Valid Card Type"),
    pin: yup.string()
      .required("PIN No is Required")
      .length(4, "PIN must be 4 digits")
  });

  const initialValues = { account: '', card_type: '', pin: '' };
              ...
              >
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-2">
                          ...
                          <div className="mb-3">
                            ...
                            <Field as="select" className="w-full py-2 border border-rose-500 rounded outline-none px-2" name="card_type" id="card_type">
                              {Types.length > 0 ? (
                                <>
                                  <option value="">Select</option>
                                  {Types.map((cur, i) => <option key={i} value={cur}>{cur}</option>)}
                                </>
                              ) : (
                                <option value="">No Card Type Available</option>
                              )}
                            </Field>
                    ...
                    {/* Display list of ATM cards */}
                    <div className="mt-4">
                      {cards.length > 0 ? (
                        <ul>
                          {cards.map(card => (
                            <li key={card._id}>
                              {card.card_type} - {card.card_no} - Expires: {new Date(card.expiry).toLocaleDateString()}
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p>No ATM Cards Yet</p>
                      )}
                    </div>
...
export default function AddNewCardDialog() {
  ...
  const accounts = Array.isArray(user.account_no)
    ...
    : user.account_no
      ? [{ _id: user.account_no, amount: user.amount ?? 0 }]
      : [];
  console.log("accounts for dropdown:", accounts)
                        ...
                        <form onSubmit={handleSubmit} className='py-4'>
                          <div className="mb-3">
                            ...
                            <Field name="account" id="account" as="select" className="w-full py-2 border border-rose-500 rounded outline-none px-2">
                              ...
                              {console.log("user.account_no:", user.account_no)}
