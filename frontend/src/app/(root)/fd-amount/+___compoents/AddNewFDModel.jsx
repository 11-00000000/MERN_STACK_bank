...
export default function AddNewFdModel({ isUpdate, setIsUpdate }) {
  ...
  const validationSchema = yup.object({
    amount: yup.number().min(1, "Minimum 1 Rs").required("Amount is required"),
  ...
  return (
    <>
      <div onClick={openModal} className='w-full border border-dashed border-blue-700 rounded shadow px-4 py-3 flex items-center justify-center flex-col hover:bg-blue-700 hover:text-white transition-all duration-300 cursor-pointer text-blue-700'>
                  ...
                  <Formik validationSchema={validationSchema} onSubmit={onSubmitHandler} initialValues={initialStates}>
                    <Form className='py-5'>
                      <div className="mb-3">
                        ...
                        <Field type="text" name='apply_for' id='apply_for' className='w-full bg-transparent border border-blue-500 rounded-md py-3 px-4 outline-none' placeholder='Enter FD Purpose...' />
